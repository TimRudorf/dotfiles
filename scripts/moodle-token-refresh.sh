...
#
# Hintergrund: Der TU-Token läuft etwa quartalsweise ab; ein Auto-Refresh über
# Shibboleth-SSO ist nicht praktikabel, der Browser-Schritt bleibt Handarbeit.
# Alles danach macht dieses Skript: prüfen, auf beide Hosts schreiben (die
# Token-Dateien haben je Host ein ANDERES Schema), zurückgelesen, verifizieren.
#
...
=====================
1. Im Browser bei Moodle per SSO einloggen, dann öffnen:
     https://$DOMAIN/user/managetoken.php
   Dort den Token des Dienstes "Moodle mobile web service" kopieren.

2. Falls die Seite keinen Token zeigt, Fallback über den Launch-Blob:
   ...
   einfügen -- das Skript dekodiert ihn selbst.
...
# Ein reiner WS-Token ist 32 Hex-Zeichen und wird unverändert durchgereicht.
...
# --- Vorab prüfen, bevor irgendetwas geschrieben wird -----------------------
printf 'Prüfe Token gegen %s ...\n' "$DOMAIN"
...
PY
)" || die "Token ungültig -- es wurde nichts geschrieben."
ok "Token gültig für $USER_INFO"
...
# Token geht über stdin, damit er nicht in der Prozessliste der VM auftaucht.
...
os.chmod(p, 0o600)
'" || die "Schreiben im Container fehlgeschlagen -- Mac ist bereits aktuell, Container läuft noch auf dem alten Token."
...
Fertig -- beide Hosts tragen denselben gültigen Token.
Nächster Schritt: der Heartbeat zieht das neue Material automatisch. Sofort
