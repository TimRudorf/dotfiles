...
**Modell (IntEco-Pilot, künftig alle Module):** Die Karten-Pläne stehen **fertig im „🎴 Anki-Karten-Plan"-Bereich jeder LE** — lean, klausur-kalibriert, gegen das Content-PDF verifiziert (Gold-Standard, siehe ). Tim arbeitet die LE durch und **triggert dann den Bau** („erstelle die Karten für LE X"). Kein interaktives Vorschlagen mehr — der Plan IST die Vorlage; nur Feinschliff passiert beim Bau. **Nie vorab bauen**.
...
> Jedes Modul ist **genau ein flaches Deck** `Uni::<Modul>`. **Nie ein Kapitel-/VO-/LE-Subdeck anlegen** — Kapitel/VO/Übung leben ausschließlich im `source::<modul>::<kapitel>`-Tag (+ `phase::<n>`). Kapitel-Cramming pre-Klausur = temporäres Filtered Deck (`deck:Uni::<Modul> tag:source::*::<kapitel>*`). Falls ein LE-Karten-Plan einen Subdeck-Namen nennt (`Uni::SDRT3::Ü3`, `Uni::Sensortechnik::VO07`, `Uni::Entrep::ef-01`, …) → ignorieren, flach ins Modul-Deck schreiben. Schema + Why:  · .
...
- **MathJax-Delimiter `\(...\)` / `\[...\]`, NIE `$...$`**: Anki rendert `$…$` per Default nicht → literale Dollarzeichen. Beim `build` einen Plan mit `$…$` oder `\\(` (Doppel-Backslash) vor dem `addNotes` normalisieren. (`\$` für Geldbeträge bleibt.) Vollregel:  „MathJax-Cloze-Syntax".
