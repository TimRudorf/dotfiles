...
bei Fehler-Reproduktion exakt den vom Melder gemeldeten Trigger/Pfad nachstellen (nicht den benachbarten); Code-Pfad vom Trigger bis Fehlerpunkt verfolgen, jeden gemeldeten Pfad einzeln testen, Beweismaterial (Logs) gegen den getesteten Pfad gegenchecken. Sonst falsches „liegt-nicht-bei-uns"-Verdikt. Volltext:

## concurrency-fix-baseline-verify

Lock-/Thread-/Recovery-Änderung unter dem **echten parallelen Szenario** live-verifizieren (grüne Units reichen nicht; ein Lock serialisiert evtl. mehr als seinen sichtbaren Zweck — eine Op aus dem Lock zu ziehen kann eine versteckte Serialisierung entfernen → Race). Roten/flaky Test nach einer Änderung erst gegen die **unveränderte Baseline** isolieren (introduced vs pre-existing, genug Wiederholungen für Aussagekraft), bevor man ihn als „flaky/pre-existing" wegerklärt. Volltext:
