...
- Env (optional): `PEXELS_TOKEN`, `OPENAI_API_KEY_PRIVATE` (mealie = privater Kontext)
- Tools: `curl`, `python3`
...
- `$ARGUMENTS` leer → nach Rezept-Pfad oder -Text fragen
...
- `OPENAI_API_KEY_PRIVATE` gesetzt → **KI-Bild generieren** (bevorzugt, direkt verwenden ohne Rückfrage)
- Nur `PEXELS_TOKEN` → **Stockfoto** (bestes Ergebnis automatisch wählen)
- Keines gesetzt → Schritt überspringen, User informieren

### KI-Bild generieren (wenn `OPENAI_API_KEY_PRIVATE` verfügbar)
