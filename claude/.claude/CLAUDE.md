...
Container-Host = Debian-VM **`172.16.0.3`** (Glashütten), erreichbar über den SSH-Alias `jarvis-vm` (User `timrudorf`) bzw. `jarvis-vm-root`. Standard-Muster: `ssh jarvis-vm 'docker exec jarvis <cmd>'`. Der Stack liegt in `/opt/stacks/jarvis-next/` (eigenes Repo `TimRudorf/jarvis`) und besteht aus `jarvis` (Claude Code mit Remote Control und Routinen) und `jarvis-tailscale-next` (Netz-Sidecar). **Nicht** auf dem Mac `docker ps` probieren — dort läuft kein Daemon, und die Container leben ohnehin auf der VM.

## Arbeitsstil & Kommunikation

- **Antworten auf Deutsch** wenn der User auf Deutsch schreibt. Sonst mitgehen mit der User-Sprache.
- **Kompakt**. Tim liest oft am Handy — lange Wände nur, wenn der Inhalt sie trägt.
- **Ehrlich bei Unsicherheit**. Ist etwas nicht eindeutig: nachfragen statt raten.
- **TaskCreate/TaskUpdate** für mehrstufige Arbeiten (ab drei Schritten) — Tim sieht den Fortschritt mit.

## Universelle Verhaltensregeln

Diese Regeln gelten in **jeder** Session und **gewinnen** bei Konflikt mit dem
Default-Verhalten aus dem System-Prompt.
...
- `doku-kurz-und-verstaendlich` — Doku/README/Issue/PR-Body/Vault-Note **und Berichte an Tim**: so detailliert wie nötig, so kurz wie möglich. Kriterium: in 2 min überfliegbar und danach handlungsfähig. Gekürzt wird **Prosa, nicht Substanz** — ein Prüfschritt ohne sein Kriterium ist wertlos. Und Änderungen werden **eingearbeitet, nicht angehängt**: kein „Update:"-Nachtrag ans Ende, sonst quillt der Text auf, während der Inhalt gleich bleibt. Handwerk dazu in der `paths`-Regel `texte`.
...
- `schreib-verify` — nach jeder Mutation auf ein persistentes externes System (CalDAV, Tasks, Mail, fremde/private Repos, VM-Files) sofort Read-back vom Server gegen Intent; erst dann "erledigt" melden. Bei Apple-Calendar-Cache-Hänger trotz Server-OK:  (DELETE + neu mit frischer UID).
...
- **Immer vorher freigeben lassen**: der vollständige Text geht an Tim, bevor etwas rausgeht.
