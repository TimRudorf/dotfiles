...
**Zugriffskette:**
- Proxmox-Host: hostname `proxmox`, user `root`
- Debian-VM darauf: `172.16.0.3`, user `timrudorf`
- Stacks-Verzeichnis in der VM: `/opt/stacks`
- **Docker-Compose läuft auf der Debian-VM unter dem User `timrudorf`, nicht root** — `ssh timrudorf@172.16.0.3` und dort `docker compose …` (nicht `ssh root@...`). Für `docker exec` ins jarvis-workspace ebenso als timrudorf.
