---
name: Cut bis Kroatien-Urlaub Juni 2026
description: Tims Bodybuilding-Cut auf Beach-Body-Niveau bis 12.06.2026 (Kroatien-Segeltörn) — Eckdaten + Plan-Ankerpunkte
type: project
originSessionId: 6da8ffce-ed73-4d31-92fe-c648997a3234
---
**Ziel:** Sichtbar shredded zum Kroatien-Segeln ab 12.06.2026 (Hinflug FRA→SPU LH 6002), Rückflug 20.06.2026. Nachhaltig — kein Crash, kein Jojo.

**Why:** Tims langfristiges Bodybuilding-Engagement, konkretes Beach-Event als externe Deadline; will Kraft im Gym halten und Substanz bewahren.

**Ausgangs-Eckdaten (Stand 2026-04-27, verifiziert via AHE/Apple Health):**
- 28 Jahre, 1,81 m, **76,3 kg** (heutige Wiegung; Vortag 77,35 kg → Wasser-/Glykogen-Rauschen, normal)
- KFA ~13–14 % geschätzt (Spiegel/Maßband, nicht Waage — siehe feedback_smartwaage_data.md)
- BMR ≈ 1763 kcal (Mifflin-St Jeor mit 76,3 kg), TDEE ≈ 3000–3100 kcal bei aktuellem Trainingsvolumen
- **Verlauf der letzten 3 Monate aus Apple Health:** Feb-Range 73–75 kg, März-Range 73–77, April 76–77 → langsame Aufwärtsdrift ~+2-3 kg seit Februar (vermutlich Bulk-Creep / Maintenance-Plus). Cut startet daher von leichtem Höchststand — gut, weil viel Wasser/Glykogen in den ersten 2 Wochen ohnehin abgeht.
- Zeitfenster: ~6,5 Wochen → realistisch 2,5–3,5 kg KG-Verlust, Ziel-Range ~72,8–73,8 kg zur Kroatien-Abreise

**Ernährungs-Anker (von Tim fix vorgegeben):**
- 500 g Magerquark abends — bleibt drin, ist Cut-mäßig perfekt (~50 g Protein, Casein, sättigend)
- Pareto-Approach: einfach, alltagstauglich
- Mittags oft auswärts mit Freunden → "flexibles Mittag", nicht streng tracken
- Alkohol nicht verboten, Wochenend-Feiern bleiben drin
- **Kein Meal-Prep aktuell** (2026-04-27 entschieden) — Mittag ist eh meistens auswärts. Falls Tim später doch will, meldet er sich.

**Tages-Plan (verabschiedet 2026-04-27):**
- **Tagesziel:** 2.400–2.500 kcal | Protein 165 g | Fett 65–70 g | Carbs ~280 g
- **Frühstück (~480 kcal, ~40 g P):** 250 g Skyr + 60 g Haferflocken + 1 Scoop Whey + Beeren + 15 g Leinsamen
- **Mittag (~700–800 kcal, flexibel auswärts):** Faustregel — vegetarischer Eiweiss-Anker (Halloumi/Falafel/Tofu/Linsen-Bowl/Eier/Caprese), Carbs ja (Reis/Bulgur/Brot/Kartoffel), keine Fritten/Sahnesoßen.
- **Snack/Pre-Workout (~250–300 kcal):** Banane + Whey ODER Hüttenkäse 200g + Reiswaffel ODER 30g Mandeln + Apfel; an Doppel-Workout-Tagen (Di/Do) mehr Carbs.
- **Abends fix:** 500 g Magerquark + 15 g Leinsamen + Beeren/1 TL Honig (~325 kcal, ~50 g P)

**Tracking (Pareto):** Tim wiegt sich **täglich morgens nüchtern** (Stand 2026-04-27). Daten landen automatisch via Renpho → Apple Health → data-api. Auswertung:
- Einzelne Tageswerte ignorieren (Wasser/Glykogen-Rauschen ±1–2 kg)
- **7-Tage-Rolling-Average** als wahre Trendmetrik
- Wochenvergleich rolling-vs-rolling
- Plus 1 Spiegel-Foto/Woche
- Bei 2 Wochen Stillstand im Rolling-Average: −150 kcal

**Training:** Split bleibt unverändert (siehe user_sport.md), Lasten halten, kein zusätzliches Cardio.

**Mikronährstoffe vegetarisch:** B12 + Eisen im Auge behalten; bei Müdigkeit Multi/B12-Spray einplanen.

**How to apply:**
- Bei Empfehlungen rund um Essen, Training, Routine: Cut-Ziel im Hinterkopf — keine "Bulk-Ratschläge", keine extremen Crash-Diäten.
- Wenn Tim nach Restaurant-Empfehlungen, Rezepten, Reise-Snacks o.ä. fragt: Kompatibilität mit Cut mit-erwähnen, ohne moralisieren.
- Plan ist **Coaching, nicht Religion** — Pareto-Prinzip; bei Slip-Ups kein Drama.
- Nach Kroatien (ab ~21.06.2026): Memory aktualisieren oder schließen, Ziele neu setzen.
